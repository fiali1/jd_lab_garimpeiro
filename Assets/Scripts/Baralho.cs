...
public class Baralho : MonoBehaviour
{
    ...
    private Carta MakeCarta(bool faceUp, int cNum) {
        ...
        if (nomesCartas[cNum].StartsWith("C")) {
            _carta.naipe = "_of_hearts";
        }

        if (nomesCartas[cNum].StartsWith("O")) {
            _carta.naipe = "_of_diamonds";
        }

        if (nomesCartas[cNum].StartsWith("E")) {
            _carta.naipe = "_of_spades";
        }

        if (nomesCartas[cNum].StartsWith("P")) {
            _carta.naipe = "_of_clubs";
        }
        ...
        else if (_carta.valor == 11) {
            numeroCarta = "jack";
        }

        else if (_carta.valor == 12) {
            numeroCarta = "queen";
        }

        else if (_carta.valor == 13) {
            numeroCarta = "king";
        }

        else {
            numeroCarta = "" + _carta.valor;
        }

        nomeCarta = numeroCarta + _carta.naipe;

        _carta.nome = "face";

        _tSp = (Sprite)(Resources.Load<Sprite>(nomeCarta));
